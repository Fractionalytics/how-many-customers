...
```
audit/
  run.py            runs every audit/sql/*.sql in order on an in-memory DuckDB
  sql/
  ...
  malloy/
    ...
    run.js          runs them
  out/              one CSV per labelled query, regenerated by run.py
  FINDINGS.md       the numbers, read
```
...
python audit/run.py            # everything
python audit/run.py 03         # one file (00_model.sql is always loaded first)
```

Each `SELECT` marked with a `-- @out <label>` comment is printed and written to
`audit/out/<file>__<label>.csv`.
...
node run.js                    # every query, in order
node run.js 05_join_folded.malloy --sql     # one query, plus the SQL Malloy generated
```
